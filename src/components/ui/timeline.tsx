...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      id="experience"
      className="w-full bg-card border border-border/60 font-sans md:px-10 rounded-2xl mt-6"
      ref={containerRef}
    >
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-lg md:pl-20 md:text-5xl font-bold text-foreground">
                {item.title}
              </h3>
            </div>
